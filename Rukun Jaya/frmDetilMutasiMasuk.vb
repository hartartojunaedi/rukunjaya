...
Public Class frmDetilMutasiMasuk
    Dim er As New rjLib
    ...
    Private Sub frmDetilMutasiMasuk_Load(sender As System.Object, e As System.EventArgs) Handles MyBase.Load
        ...
        If insert = False Then
            ...
            cmd.Connection = cn
            cmd.CommandText = "Select tgltransaksi,namatoko from thmasuk where nota=@nota"
    ...
    Sub loaddata()
        Dim cmd As SqlCommand = New SqlCommand
        cmd.Connection = cn
        cmd.CommandText = "select namasparepart from tsparepart where kodesparepart=@kode"
    ...
    Sub panggil()
        Try
            ...
            cmd.Connection = cn
            cmd.CommandText = "Select i.kodesparepart,i.namasparepart,d.jumlah from tdmasuk d, tSparepart i  where i.kodesparepart=d.kodesparepart and d.nota=@nota"
            cmd.Parameters.AddWithValue("@nota", txtNonota.Text)
            cn.Open()
            reader = cmd.ExecuteReader()
            While reader.Read()

                Dim row As String() = New String() {reader.Item(0), reader.Item(1), reader.Item(2), 0}
    ...
    Private Sub btnBrowse_Click(sender As System.Object, e As System.EventArgs) Handles btnBrowse.Click
        frmCariSparepart.mode = 2
        frmCariSparepart.ShowDialog()
    End Sub

    Private Sub frmDetilMutasiMasuk_KeyDown(sender As System.Object, e As System.Windows.Forms.KeyEventArgs) Handles MyBase.KeyDown
    ...
    Sub updatebaru()
        ...
        Try
            ...
            cmd.CommandText = "Insert into thmasuk(nota,tgltransaksi,namatoko) values (@nota,@tgltrans,@namatoko)"
            cmd.Parameters.AddWithValue("@nota", txtNonota.Text)
            ...
            For i = 0 To dgvdetilpembelian.RowCount - 1
                cmd.Parameters.Clear()
                cmd.CommandText = "Insert into tdmasuk(nota,kodesparepart,jumlah) values (@nota,@kodesp,@jumlah)"
                ...
                If hargadb <> dgvdetilpembelian.Rows(i).Cells("harga").Value Then
                    ...
                    'cmd.ExecuteNonQuery()
                    cmd.CommandText = "Insert into thistoryharga(kodesparepart,harga,tgl) values (@kodesp,@harga,@tgltrans)"
                    'cmd.ExecuteNonQuery()
            ...
            rj.isilog(rj.getusername, txtNonota.Text, "Transaksi Mutasi Masuk", "Update Data", log)
        ...
        Catch ex As Exception
            ...
            rj.isiErrorlog(rj.getusername, txtNonota.Text, "Transaksi Mutasi Masuk", "Update Data", ex.Message)
    ...
    Sub insertbaru()
        ...
        cmd1.CommandText = "Select count(nota) from thmasuk where nota=@nota"
            ...
            If txtNonota.Text.Length > 0 And txtNamaToko.Text.Length > 0 Then
                If dgvdetilpembelian.RowCount > 0 Then
                    ...
                    Try
                        ...
                        Next
                        Dim log As String = "" ''"nota = " & txtNonota.Text & vbCrLf & " nama toko =" & txtNamaToko.Text & vbCrLf & " total =" & lblTotal.Text & vbCrLf & " grand total setelah diskon =" & lblgrandtotal.Text
                        rj.isilog(rj.getusername, txtNonota.Text, "Transaksi Mutasi Masuk", "Tambah Data", log)
                        rj.shownotify("Data Berhasil Disimpan")
                    ...
                    Catch ex As Exception
                        ...
                        cn.Close()
                        'Dim log As String = "nota = " & txtNonota.Text & vbCrLf & " nama toko =" & txtNamaToko.Text & vbCrLf & " total =" & lblTotal.Text & vbCrLf & " grand total setelah diskon =" & lblgrandtotal.Text & " Error message  = " & ex.Message
                        rj.isiErrorlog(rj.getusername, txtNonota.Text, "Transaksi Mutasi Masuk", "Tambah Data", ex.Message)
                        rj.shownotifyerror("Data Tidak Berhasil Disimpan")
    ...
    Private Sub btnTambah_Click(sender As System.Object, e As System.EventArgs) Handles btnTambah.Click
        Dim row As String() = New String() {varkodesparepart, txtKodebarang.Text, nmJumlah.Value}
