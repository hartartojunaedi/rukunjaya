...
Public Class frmDetailSuratJalan
    ...
    Sub tampilbaiaya()
        ...
        If (txtnosuratjalan.Text = "" And lblMode.Text.ToLower = "tambah") Then
            cmd.CommandText = "select db.kodebiaya as tkodebiaya,b.namabiaya as tnamabiaya,db.biaya as tbiaya,db.nobm as tnobm,db.nosuratjalan as tnosuratjalan,db.keterangan as tketerangan from tbiaya b, tdetailbiaya db where db.kodebiaya=b.kodebiaya and 1=2"
        Else
            cmd.CommandText = "select db.kodebiaya as tkodebiaya,b.namabiaya as tnamabiaya,db.biaya as tbiaya,db.nobm as tnobm,db.nosuratjalan as tnosuratjalan,db.keterangan as tketerangan from tbiaya b, tdetailbiaya db where db.kodebiaya=b.kodebiaya and nobm=@nobm and nosuratjalan=@nosj"
        ...
        'Dim builder As SqlCommandBuilder = New SqlCommandBuilder(adapter)
        adapter1.Fill(ds, "tbiaya")
        'jun 15 jan 2015
        dgvBiaya.AutoGenerateColumns = False
        dgvBiaya.DataSource = ds.Tables("tbiaya")
        dgvBiaya.Refresh()
        conn.Close()
    End Sub
    Sub tampilvoucher()
    ...
    Private Sub updatedata()
        ...
        If _rjcommon.cekstatussuratjalan(txtnosuratjalan.Text) = 0 Then
            Try
                ...
                For j As Integer = 0 To dgvBiaya.RowCount - 1
                    cmd.Parameters.Clear()
                    cmd.CommandText = "insert into tdetailbiaya(kodebiaya,nobm,nosuratjalan,biaya,keterangan) values(@kodebiaya,@nobm,@nosuratjalan,@biaya,@keterangan)"
                    cmd.Parameters.AddWithValue("@kodebiaya", dgvBiaya.Rows(j).Cells("tkodebiaya").Value)
                    cmd.Parameters.AddWithValue("@nobm", nobm)
                    cmd.Parameters.AddWithValue("@nosuratjalan", txtnosuratjalan.Text)
                    cmd.Parameters.AddWithValue("@biaya", dgvBiaya.Rows(j).Cells("tbiaya").Value)
                    cmd.Parameters.AddWithValue("@keterangan", dgvBiaya.Rows(j).Cells("tketerangan").Value)
                    cmd.ExecuteNonQuery()
                Next
                ...
                If _rjcommon.cekvoucherbbm(nobm, txtnosuratjalan.Text) = 0 Then
                    ...
                    For j As Integer = 0 To dgvvoucher.RowCount - 1
                        ...
                        biayasolar += dgvvoucher.Rows(j).Cells("rupiah").Value
                        cmd.ExecuteNonQuery()
                        'Else
                        'cmd.Parameters.Clear()
                        'cmd.CommandText = "update tdetailsuratjalan set nodo=@nodo,jumlah=@jumlah,noshippinglist=@noshippinglist where nobm=@nobm and nosuratjalan=@nosuratjalan and kodebarang=@kodebarang"
                    ...
                    Next
                    If biayasolar > 0 Then
                        cmd.Parameters.Clear()
                        cmd.CommandText = "insert into tdetailbiaya(kodebiaya,nobm,nosuratjalan,biaya,keterangan) values(@kodebiaya,@nobm,@nosuratjalan,@biaya,@keterangan)"
                        'set ongkos angkut
                        cmd.Parameters.AddWithValue("@kodebiaya", "x004")
                        cmd.Parameters.AddWithValue("@nobm", nobm)
                        cmd.Parameters.AddWithValue("@nosuratjalan", txtnosuratjalan.Text)
                        cmd.Parameters.AddWithValue("@biaya", biayasolar)
    ...
    Private Sub tambahdata()
        Try
            ...
            If i = 0 Then
                ...
                '-----ini untuk masukkan detailbiaya
                For j As Integer = 0 To dgvBiaya.RowCount - 1
                    cmd.Parameters.Clear()
                    cmd.CommandText = "insert into tdetailbiaya(kodebiaya,nobm,nosuratjalan,biaya,keterangan) values(@kodebiaya,@nobm,@nosuratjalan,@biaya,@keterangan)"
                    cmd.Parameters.AddWithValue("@kodebiaya", dgvBiaya.Rows(j).Cells("tkodebiaya").Value)
                ...
                cmd.CommandText = "insert into tdetailbiaya(kodebiaya,nobm,nosuratjalan,biaya,keterangan) values(@kodebiaya,@nobm,@nosuratjalan,@biaya,@keterangan)"
                'set ongkos angkut
                cmd.Parameters.AddWithValue("@kodebiaya", "x001")
                cmd.Parameters.AddWithValue("@nobm", nobm)
                cmd.Parameters.AddWithValue("@nosuratjalan", txtnosuratjalan.Text)
    ...
    Private Sub Button1_Click(sender As System.Object, e As System.EventArgs) Handles Button1.Click
        Dim err As String = 0
        'jun 24 mar 2015
        If Not IsNumeric(txtbiaya.Text) Then
            MsgBox("Isian harus berupa angka", MsgBoxStyle.OkOnly, "Information")
            err = 1
        End If
        ...
        End If
        If err <> 1 Then
            Dim tr As DataRow = ds.Tables("tbiaya").NewRow
            tr("tnobm") = nobm
            tr("tnosuratjalan") = txtnosuratjalan.Text
            tr("tkodebiaya") = cmbBiaya.SelectedValue
            tr("tnamabiaya") = cmbBiaya.Text

            tr("tbiaya") = txtbiaya.Text
            tr("tketerangan") = txtketbiaya.Text
            ds.Tables("tbiaya").Rows.Add(tr)
            clearbiaya()
        End If
    End Sub
    ...
    Private Sub dgvBiaya_CellClick(sender As System.Object, e As System.Windows.Forms.DataGridViewCellEventArgs) Handles dgvBiaya.CellClick
        If (e.ColumnIndex = 6) Then
            If (MsgBox("Apakah anda yakin menghapus data biaya ?", MsgBoxStyle.YesNo, "Konfirmasi hapus") = MsgBoxResult.Yes) Then
                ds.Tables("tbiaya").Rows(e.RowIndex).Delete()
            End If
        End If
    End Sub
End Class